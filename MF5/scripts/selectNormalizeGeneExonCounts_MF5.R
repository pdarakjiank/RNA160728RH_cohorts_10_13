...
source("MF5/scripts/functionDefinitions.R")
try(dir.create("MF5/analysis/resultsCoexpr_MF5"), silent = F)
try( dir.create("MF5/analysis/figuresCoexpr_MF5"), silent = F)
try(dir.create("MF5/analysis/resultsCoSplicEx_MF5"), silent = F)
try( dir.create("MF5/analysis/figuresCoSplicEx_MF5"), silent = F)

# read raw data - can be improved by using read.table from original .txt file
exonReadsRaw=read.table("MF5/data/RNA160728RH_Cy_coverage_splitoption_stranded_oppositeDirection.txt",header=F,sep="\t",stringsAsFactors = F)
...
exon_counts<-ddply(exon_counts, 12, numcolwise(sum)) #for some reason a few same name exons in both strands (one strand with close to zero reads)
rownames(exon_counts)<-exon_counts$exon

# Remove the gene symbol and exon id columns from the exon data frame
exon_counts<-exon_counts[,2:11]
...
gene_counts<-gene_counts[,2:11]

# Load sample names in the order they are in the coverage file
samples <- read.table("MF5/scripts/MF5_bam_files.txt",header=F,stringsAsFactors = F)
...
write.table(gene_counts,"MF5/analysis/RNA160728RH_MF5_gene_reads_not_normalized.txt", sep="\t",quote=F,col.names=T,row.names=T)
write.table(exon_counts,"MF5/analysis/RNA160728RH_MF5_exon_reads_not_normalized.txt", sep="\t",quote=F,col.names=T,row.names=T)
save.image("MF5/data/Read_Counts_and_Normalization_MF5.RData")
...
write.table(normalizedGeneCountsUQ_exons,"MF5/analysis/RNA160728RH_MF5_exon_reads_UQNormalized.txt", sep="\t",quote=F,col.names=T,row.names=T)
write.table(normalizedGeneCountsUQ_genes,"MF5/analysis/RNA160728RH_MF5_gene_reads_UQNormalized.txt", sep="\t",quote=F,col.names=T,row.names=T)
save.image("MF5/data/Read_Counts_and_Normalization_MF5.RData")


########################################################################################
# IF YOU NEED TO CALCULATE DIFFERENTIAL EXPRESSION:
load("MF5/data/Read_Counts_and_Normalization_MF5.RData")

# read sample info
sampleKey = read.table("MF5/data/rhesus_phenotype.txt",header=F,sep="\t")
...
save(samplesControls,samplesDrinkers,countsControls,countsDrinkers,sampleKey, file="DE_MF5.RData")
...
fileConnSummary<-file("resultsCoexpr_MF5/SummaryResultsCoexpr.txt",  open="wt")
...
fileConnSummary<-file("resultsCoexpr_MF5/SummaryResultsCoexpr.txt",  open="at")
...
write.csv(geneNamesDE, file="resultsCoexpr_MF5/geneNamesDE.csv")
write.csv(geneNamesDV, file="resultsCoexpr_MF5/geneNamesDV.csv")
...
write.csv(results_highCPMgenes, file="resultsCoexpr_MF5/resultsDEDV_highCPM_MF5.csv")
