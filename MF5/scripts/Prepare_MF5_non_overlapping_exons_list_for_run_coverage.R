...
gtf<-read.delim("/lawrencedata/sharedRNASeq/Cyno_reference_genome/mf5/CY.v6.stable_sorted.gtf", sep="\t", header=FALSE, stringsAsFactors=FALSE)
gtf <- gtf[gtf$V3 == "exon",]

# Extract gene symbol id field (3) from column 9 of gtf
sym.gene.name.gtf<-sapply(strsplit(gtf$V9,";"),function(x) x[4])
...
gtf$gene.sym<-gsub(" gene_name ","",gtf$gene.sym)

# In order to generate a GRanges object the strand "." needs to be replaced by "*"
gtf$V7<-gsub("\\.","*",gtf$V7)
...
# Verify if there are overlaps between exons of different genes
...
##### NOTE: For this MF5 annotation there is no overlap of exons transripts between different genes
